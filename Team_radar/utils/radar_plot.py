...
class Radar:
    ...
    def plot_radar(self, ranges, params, values, radar_color, filename=None, dpi=300,
        ...
        ax = self.__add_labels(params=params, ax=ax, num=1)
        ...
        # ax.text(-22.5,-21.5,'FOOTSCI', fontsize=18, fontweight='bold', fontdict={'color': 'black', 'font':'serif'})
        ...
        if endnote != None:
            ...
            for note in endnote.split('\n'):
                ax.text(-22, y_add, note, fontsize=20, fontweight='bold', fontdict={'color': 'black', 'font':'serif'})
                y_add -= 1
    ...
    def __plot_titles(self, ax, title):
        ...
        if title.get('subtitle_name_2'):
            ...
            ax.text(22, 20, title['subtitle_name_2'], fontsize=title['subtitle_fontsize_2'], 
                    fontdict={'color': title['subtitle_color_2']}, ha='right', fontfamily=self.fontfamily) 

        if title.get('title_description'):
            ## plot the second subtitle name
            ax.text(0, 21, title['title_description'], fontsize=title['title_description_fontsize'], 
                    fontdict={'color': title['title_description_color']}, ha='center', fontfamily=self.fontfamily, 
                    fontweight='bold')
    ...
    def __add_labels(self, params, ax, num, return_list=False, radius=19, range_val=False):
        """
        ...
        for i in range(len(params)):
            ...
            if type(params[i]) == np.float64:
                ...
                if p % 1 == 0:
                    ...
                    # print(p)
#                 try:
#                     global c = round(params[i], 1)
#                     global c_next = round(params[i+13], 1)
#                     if p_next == p:
#                         p = round(params[i], 3)
...
#                 print(p)
            ax.text(x, y, p, rotation=-np.rad2deg(rot), ha='center', va='center', 
                  ...
                  fontweight = 'bold' if type(p) == str else  'normal')
    ...
    def __add_ranges(self, ranges, ax):
        ...
        for rng in ranges:
            ...
            r = 0
            while(sum_ != 0):
                sum_1 = np.array([abs(round(value[i+1],0) - round(value[i],0)) > 1 for i in range(len(value) - 1)]).sum()
                sum_2 = np.array([round(value[i+1],0) > 20 for i in range(len(value) - 1)]).sum()
                if (sum_1 !=0) or (sum_2 !=0):
                    sum_ = 0
                else:
                    r = r + 1
                    sum_ = np.array([round(value[i],r) == round(value[i+1],r) for i in range(len(value) - 1)]).sum()
            
            if r!=0:
                value = np.array(value).round(r)
            else:
        ...
        for i in range(len(radius)):
            ...
            ax, xy = self.__add_labels(params=params, ax=ax, return_list=True, radius=radius[i], range_val=True, num=i)
            x_y.append(xy)
